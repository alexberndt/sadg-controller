...
class Environment {
    ...
    neighbors.clear();
    {
      ...
      if (stateValid(n) && transitionValid(s, n)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(n, Action::Wait, 1)); // 99 = COST OF EDGE TRAVERSAL -> IT WAS 1 ORIGINALLY
      }
    }
    {
      ...
      if (stateValid(n) && transitionValid(s, n)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(n, Action::Left, 1)); // 100 = COST OF EDGE TRAVERSAL -> IT WAS 1 ORIGINALLY
      }
    }
    {
      State n(s.time + 1, s.x + 1, s.y);
      if (stateValid(n) && transitionValid(s, n)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(n, Action::Right, 1)); // 100 = COST OF EDGE TRAVERSAL -> IT WAS 1 ORIGINALLY
      }
    }
    {
      ...
      if (stateValid(n) && transitionValid(s, n)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(n, Action::Up, 1)); // 100 = COST OF EDGE TRAVERSAL -> IT WAS 1 ORIGINALLY
      }
    }
    {
      State n(s.time + 1, s.x, s.y - 1);
      if (stateValid(n) && transitionValid(s, n)) {
        neighbors.emplace_back(
            Neighbor<State, Action, int>(n, Action::Down, 1)); // 100 = COST OF EDGE TRAVERSAL -> IT WAS 1 ORIGINALLY
      }
    }
  }
 ...
 private:
  ...
  // m_constraints:
  // vertexConstraints: std::unordered_set<VertexConstraint>
...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()("help", "produce help message")
      ("map,m", po::value<std::string>(&mapFile)->required(),
      "map file (YAML)")
      ("robots,r", po::value<std::string>(&robotFile)->required(),
      "robots' start and goal (YAML)")
  ...
  YAML::Node mapConfig = YAML::LoadFile(mapFile);
  YAML::Node robotConfig = YAML::LoadFile(robotFile);

  std::unordered_set<Location> obstacles;
  std::vector<Location> goals;
  std::vector<State> startStates;

  const auto& dim = mapConfig["map"]["dimensions"];
  int dimx = dim[0].as<int>();
  int dimy = dim[1].as<int>();

  for (const auto& node : mapConfig["map"]["obstacles"]) {
    obstacles.insert(Location(node[0].as<int>(), node[1].as<int>()));
  }

  for (const auto& node : robotConfig["robots"]) {
