...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()("help", "produce help message")
      ("map file,m", po::value<std::string>(&mapFile)->required(),
      "map data file (YAML)")
      ("robot file,r", po::value<std::string>(&robotFile)->required(),
      "robot locations (YAML)")
  ...
  YAML::Node configMap = YAML::LoadFile(mapFile);

  std::unordered_set<Location> obstacles;

  const auto& dim = configMap["map"]["dimensions"];
  int dimx = dim[0].as<int>();
  int dimy = dim[1].as<int>();

  for (const auto& node : configMap["map"]["obstacles"]) {
    obstacles.insert(Location(node[0].as<int>(), node[1].as<int>()));
  }

  YAML::Node configRobot = YAML::LoadFile(robotFile);

  std::vector<Location> goals;
  std::vector<State> startStates;

  for (const auto& node : configRobot["robots"]) {
